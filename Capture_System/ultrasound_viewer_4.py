...
包含完整的 RF 数据重建、希尔伯特变换包络提取和 TGC 补偿。
"""
...
class UltrasoundProcessor:
    def __init__(self, tgc_slope: float = 0.0,
        ...
        self.dynamic_range = dynamic_range  # 动态范围 dB

    def preprocess(self, raw: np.ndarray) -> np.ndarray:
        """
        raw: (N_frames, N_samples) float32 原始带直流偏置的 RF 数据
        """
        raw_float = raw.astype(np.float32)
        n_frames, n_samples = raw_float.shape

        # 1. 去除直流偏置 (DC Removal)
        # ADC 采样出来的是 50~210 左右的震荡数值，减去每帧的均值使其重回 0 轴
        rf_centered = raw_float - np.mean(raw_float, axis=1, keepdims=True)

        # 2. 希尔伯特变换提取包络 (Hilbert Transform)
        # 在深度维度（axis=1）上计算解析信号，然后取模值（绝对值）
        analytic_signal = hilbert(rf_centered, axis=1)
        envelope = np.abs(analytic_signal)

        # 3. 对数压缩 (Log Compression)
        envelope = np.clip(envelope, 1e-6, None) # 防止 log(0)
        log_env  = 20.0 * np.log10(envelope)

        # 4. 时间增益补偿 (TGC)
        if self.tgc_slope != 0.0:
            tgc_gain = np.arange(n_samples, dtype=np.float32) * self.tgc_slope
            log_env += tgc_gain[np.newaxis, :]

        # 5. 2D 高斯平滑 (去斑点噪声与时序连贯)
        if self.sigma_depth > 0 or self.sigma_time > 0:
            smoothed = gaussian_filter(log_env, sigma=[self.sigma_time, self.sigma_depth])
        else:
            smoothed = log_env

        # 6. 动态范围截断与归一化 -> [0, 1] 映射到灰度
        vmax = smoothed.max()
...
class UltrasoundViewer(QWidget):
    def __init__(self, csv_file: str = ""):
        super().__init__()
        self.setWindowTitle("超声灰度图查看器 — M-mode (含 Hilbert 包络提取)")
        self.resize(1200, 800)

        self._raw_frames:  dict[int, np.ndarray] = {}   
        self._proc_frames: dict[int, np.ndarray] = {}   
    ...
    def _build_ui(self):
        ...
        self._spin_s0 = QSpinBox(); self._spin_s0.setRange(0, 9999);  self._spin_s0.setValue(0);    self._spin_s0.setFixedWidth(60)
        self._spin_s1 = QSpinBox(); self._spin_s1.setRange(1, 9999); self._spin_s1.setValue(1000); self._spin_s1.setFixedWidth(60)
        ...
        self._spin_tgc.setRange(0.0, 0.5); self._spin_tgc.setValue(0.02) # 给了点默认的 TGC
        self._spin_tgc.setDecimals(4); self._spin_tgc.setSingleStep(0.01); self._spin_tgc.setFixedWidth(75)
        row2.addWidget(self._spin_tgc)

        row2.addWidget(QLabel("动态范围(dB):"))
        self._spin_dr = QDoubleSpinBox()
        self._spin_dr.setRange(1.0, 100.0); self._spin_dr.setValue(35.0) # 调大了动态范围
    ...
    def _load_file(self, path: str):
        ...
        self._spin_s1.setValue(self._n_samples_max)
    ...
    def _reset_bufs(self):
        self._bufs = {ch: np.zeros((self._history, self._n_samples_max), dtype=np.float32)
                      for ch in self._proc_frames}

    def _render_img(self, ch: int, buf: np.ndarray):
        s0 = max(0, int(self._spin_s0.value())); s1 = min(self._n_samples_max, int(self._spin_s1.value()))
        if s1 <= s0: s1 = s0 + 1
