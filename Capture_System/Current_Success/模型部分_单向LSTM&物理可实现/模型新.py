...
CONFIG = {
    ...
    "save_dir": r"D:\nightmare\Documents\SRTP\LSTM\training_logs\ultr_fusion_model_new_auto_1",
    ...
    # 严格禁止 i+1 使用（因果化中值/并在选择时回退非因果策略）。
    "strict_disable_i1_usage_enable": True,
...
def suppress_phase_spikes(phase: np.ndarray, mad_k: float, abs_step_th: float) -> np.ndarray:
    ...
    if bool(CONFIG.get("strict_disable_i1_usage_enable", False)):
        # Causal median over [i-2, i-1, i] steps (no i+1 lookahead).
        pad = np.pad(steps, (2, 0), mode="edge")
        med = np.median(np.stack([pad[:-2], pad[1:-1], pad[2:]], axis=1), axis=1).astype(np.float32)
    else:
        # Centered median over [i-1, i, i+1] steps.
...
def _smooth_hybrid_ensemble(base, preds_raw: np.ndarray) -> np.ndarray:
    ...
    vel_low, vel_high = float(CONFIG.get("adaptive_vel_low", 0.035)), float(CONFIG.get("adaptive_vel_high", 0.120))
...
def smooth_predictions(base, preds_raw, strategy=None):
    ...
    # Strict causal mode: block non-causal whole-sequence reconstructions.
    if bool(CONFIG.get("strict_disable_i1_usage_enable", False)):
        if strategy in {"perfect_sawtooth", "anchor_sync"}:
            strategy = "adaptive_tracker"
    ...
    if strategy == "perfect_sawtooth":
        ...
        if k_size >= 3: steps = medfilt(steps, kernel_size=k_size)
            
        win = min(15, len(steps))
        if win >= 3: steps = np.convolve(steps, np.ones(win)/win, mode='same')
            
        reconstructed = np.cumsum(steps)
        drift = unwrapped - reconstructed
        if len(drift) > 15:
            b, a = butter(N=2, Wn=0.015, btype="low")
            smooth_drift = filtfilt(b, a, drift)
        else: smooth_drift = drift
            
        final_unwrapped = reconstructed + smooth_drift + 0.015
    ...
    if strategy == "anchor_sync":
        ...
        if len(raw_unwrapped) > 15:
            ...
            smooth_unwrapped = filtfilt(b, a, raw_unwrapped)
        else: smooth_unwrapped = raw_unwrapped
            
        smooth_phase = smooth_unwrapped % 1.0
        ...
        avg_cycle = np.mean(np.diff(anchors)) if len(anchors) > 1 else len(raw_phase) / max(0.5, (smooth_unwrapped[-1] - smooth_unwrapped[0]))
        
        first_anchor = anchors[0]
        if first_anchor > 0: out_phase[:first_anchor] = (1.0 - np.arange(first_anchor, 0, -1) / avg_cycle) % 1.0
            
        last_anchor = anchors[-1]
        if last_anchor < len(raw_phase): out_phase[last_anchor:] = (np.arange(0, len(raw_phase) - last_anchor) / avg_cycle) % 1.0
